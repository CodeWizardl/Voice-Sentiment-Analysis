...
if save_location:
    ...
    if not grouped.empty:
        # Check if 'POSITIVE' sentiment is present in the 'grouped' DataFrame
        if 'POSITIVE' in grouped['sentiment'].values:
            pos_perc = grouped[grouped['sentiment'] == 'POSITIVE']['count'].iloc[0] * 100 / sen_df.shape[0]
        else:
            pos_perc = 0  # or any default value if 'POSITIVE' sentiment is not present

        # Check if 'NEGATIVE' sentiment is present in the 'grouped' DataFrame
        if 'NEGATIVE' in grouped['sentiment'].values:
            neg_perc = grouped[grouped['sentiment'] == 'NEGATIVE']['count'].iloc[0] * 100 / sen_df.shape[0]
        else:
            neg_perc = 0  # or any default value if 'NEGATIVE' sentiment is not present

        # Check if 'NEUTRAL' sentiment is present in the 'grouped' DataFrame
        if 'NEUTRAL' in grouped['sentiment'].values:
            neu_perc = grouped[grouped['sentiment'] == 'NEUTRAL']['count'].iloc[0] * 100 / sen_df.shape[0]
        else:
            neu_perc = 0  # or any default value if 'NEUTRAL' sentiment is not present
    else:
        # Handle the case where the 'grouped' DataFrame is empty
        pos_perc, neg_perc, neu_perc = 0, 0, 0
